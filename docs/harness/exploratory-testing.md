...
5. **セッションログがなければ探索は未完了**。探索開始時にログファイルを作り、probeを実施するたびに
   観測を追記する。チャーター、全probe（未確認を含む）、振り分け、価値評価が揃うまで、エージェントは
   探索完了を報告してはならない。会話履歴、スクリーンショット、コミットメッセージだけを記録の代替にしない。
6. **各findingから恒久テストまで追跡可能にする**。セッションログにはfinding ID、テストファイルと
   テストメソッド名、確認する仕様、反映状態を対応づける。テストへ蒸留しないfindingには、その理由と
   代替の着地点を記録する。
...
1. チャーターを書いたセッションログを `docs/harness/exploration-sessions/` に作成する。
...
4. probeごとに、成功・発見・未確認をセッションログへ直ちに記録する（様式は次節）。
5. 発見を3方向に振り分ける（振り分け基準は次々節）。
6. findingと恒久テストの対応表を記入する。テスト化しないfindingには理由を記入する。
7. 機械可読な価値評価を記入してから探索完了を報告する。
...
## 回帰テストへの蒸留
| finding | 回帰テスト（クラス.メソッド） | 確認する仕様 | 状態または未反映理由 |
## 次のチャーター候補
（このセッションで見えた未探索領域）
```
