...
| P8 | 登録状態の再起動復元 | 登録済みと削除済みの各状態でforce-stop・起動 | 一覧の動作表示は登録後に復元され、削除後は「オフ」を維持した。復元形状による実際の発火は未確認 | 規則どおり（一部未確認） | 一覧復元は実機確認済み、発火は次回へ継続 |
...
P8の一覧復元は確認済みでfindingではない。復元した形状による実際の発火は未確認であり、次回観測後に
欠陥と判定した場合はこの表へ回帰テストを追加する。

## 次のチャーター候補

- 登録済みカスタムジェスチャーについて、再起動後に同じ形状を描いて動作が実際に発火することを確認する。
