...
- Code highlighting and basic local Mermaid diagram rendering
...
LocalMD Reader v0.1.0 focuses on viewing, not editing. Full CommonMark compatibility, advanced Mermaid controls, math, footnotes, remote image loading, relative image rendering, and cloud sync are not included in this version.
```
...
```text
docs/guides/icon-design-note.md
```
...
```text
docs/release/play-store-screenshots.md
```
