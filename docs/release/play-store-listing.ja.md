...
- コードハイライトと基本的な Mermaid 図のローカル表示
...
LocalMD Reader v0.1.0 は編集ではなく閲覧に集中しています。完全な CommonMark 互換、Mermaid の高度な操作、数式、脚注、リモート画像読み込み、相対画像表示、クラウド同期はこのバージョンでは含まれません。
```
...
- プライバシーに関する表現は `PRIVACY.ja.md` と
  `docs/release/play-store-data-safety.ja.md` に合わせる。
...
```text
docs/guides/icon-design-note.ja.md
```
...
```text
docs/release/play-store-screenshots.ja.md
```
