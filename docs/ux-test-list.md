# UX Test List

## First Run

- [x] Welcome screen explains how to open a Markdown file.
- [x] Welcome screen mentions Recent files, tabs, and pinch font sizing.
- [x] Welcome screen repeats the no ads, no tracking, no network promise.

## Navigation

- [x] Top bar shows the app name.
- [x] Menu button label includes the word `Menu`.
- [x] Drawer includes short product context.
- [x] Drawer actions are grouped by Files, Reading, and Layout.

## Feedback

- [x] Empty Recent files state appears in a dialog.
- [x] Clear recent files result appears in a dialog.
- [x] File open failures appear in dialogs with actionable reasons.

## Tabs

- [x] Active tab uses stronger visual emphasis than inactive tabs.
- [x] Long tab titles are constrained to avoid breaking the tab row.
- [x] Close tab controls have accessibility labels.
- [x] Close tab control is hidden when there is only one tab.
