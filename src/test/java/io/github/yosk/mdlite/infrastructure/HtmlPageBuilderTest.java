    ...
    final class PageLayout {
        ...
        void rendersLightThemeWithLightBackgroundAndDarkText() {
            String page = HtmlPageBuilder.buildPage(
                    SafeHtml.fromTrustedRendererOutput("<h1>Title</h1>"), ViewerTheme.light());
        ...
        void bodyCentersOnWideViewports() {
            ...
            TestAssertions.assertContains(page, "body{font-family:sans-serif;margin:0 auto;",
        ...
        void headingScaleCoversH3ToH6() {
            String page =
                    HtmlPageBuilder.buildPage(SafeHtml.fromTrustedRendererOutput("<h3>Sub</h3>"), ViewerTheme.light());
            ...
            TestAssertions.assertContains(
                    page, "--localmd-h4-font-size:17px;", "h4 must scale from the body size (16+1)");
            ...
            TestAssertions.assertContains(
                    page, "h6{color:#566664;}", "h6 must use the muted text color to mark the lowest heading level");
        }
        ...
        void fontSizeUpdateScriptKeepsTheWholeHeadingScale() {
            ...
            TestAssertions.assertContains(
                    script, "'--localmd-h3-font-size','23px'", "pinch zoom must rescale h3 along with the body (#73)");
            TestAssertions.assertContains(
                    script, "'--localmd-h4-font-size','21px'", "pinch zoom must rescale h4 along with the body");
        }
    }

    @Nested
    final class ThemeSurfaces {
        @Test
        void rendersDarkThemeWithDarkBackgroundAndLightText() {
            String page =
                    HtmlPageBuilder.buildPage(SafeHtml.fromTrustedRendererOutput("<h1>Title</h1>"), ViewerTheme.dark());
        ...
        void rendersAmoledThemeWithBlackBackgroundAndLightText() {
            String page = HtmlPageBuilder.buildPage(
                    SafeHtml.fromTrustedRendererOutput("<h1>Title</h1>"), ViewerTheme.amoled());

            TestAssertions.assertContains(
                    page, "background:#000000", "AMOLED theme must use a true black page background");
        ...
        void rendersGradientThemeWithModernReadableBackground() {
            String page = HtmlPageBuilder.buildPage(
                    SafeHtml.fromTrustedRendererOutput("<h1>Title</h1>"), ViewerTheme.gradient());

            TestAssertions.assertContains(page,
                    "background:linear-gradient(135deg,#f3f7ff 0%,#d9f2ec 32%,#ffe2d1 68%,#f8f0ff 100%)",
                    "Gradient theme must use a distinct modern reading background");
        ...
        void rendersAuroraThemeWithModernReadableBackground() {
            String page = HtmlPageBuilder.buildPage(
                    SafeHtml.fromTrustedRendererOutput("<h1>Title</h1>"), ViewerTheme.aurora());

            TestAssertions.assertContains(page,
                    "background:linear-gradient(135deg,#081411 0%,#123d36 34%,#0a5c58 64%,#3a5b2a 100%)",
                    "Aurora theme must use a distinct dark aurora reading background");
        ...
        void rendersMistThemeWithModernReadableBackground() {
            String page =
                    HtmlPageBuilder.buildPage(SafeHtml.fromTrustedRendererOutput("<h1>Title</h1>"), ViewerTheme.mist());

            TestAssertions.assertContains(
                    page, "background:#ffffff", "Mist theme must use a high-contrast plain reading background");
        ...
        void rendersDuskThemeWithModernReadableBackground() {
            String page =
                    HtmlPageBuilder.buildPage(SafeHtml.fromTrustedRendererOutput("<h1>Title</h1>"), ViewerTheme.dusk());

            TestAssertions.assertContains(page,
                    "background:linear-gradient(135deg,#f4ead7 0%,#ead8b8 52%,#f8f0df 100%)",
                    "Dusk theme must use a sepia long-form reading background");
            TestAssertions.assertContains(page, "color:#2a2118", "Dusk theme must keep dark readable text");
        }
    }

    @Nested
    final class ReadingCanvas {
        @Test
        void addsBottomReadingSpaceAfterLastLine() {
            String page = HtmlPageBuilder.buildPage(
                    SafeHtml.fromTrustedRendererOutput("<p>Last line</p>"), ViewerTheme.light());

            TestAssertions.assertContains(page, "padding:24px 24px 40vh;",
                    "document body must add bottom reading space so the final line can scroll above the screen bottom");
        }
        ...
        void welcomePrimaryActionUsesSolidTextColorOnGradientThemes() {
            String page = HtmlPageBuilder.buildPage(
                    SafeHtml.fromTrustedRendererOutput(
                            "<section class=\"welcome\"><a class=\"welcome-primary-action\">Open Markdown "
                            + "file</a></section>"),
        ...
        void includesRenderedMarkdownBodyUnchanged() {
            String page = HtmlPageBuilder.buildPage(
                    SafeHtml.fromTrustedRendererOutput("<h1>Title</h1><p>Body</p>"), ViewerTheme.light());

            TestAssertions.assertContains(
                    page, "<h1>Title</h1><p>Body</p>", "page must include rendered Markdown body unchanged");
        }
    }

    @Nested
    final class DynamicTypography {
        @Test
        void appliesFontSizeToParagraphText() {
            String page = HtmlPageBuilder.buildPage(
                    SafeHtml.fromTrustedRendererOutput("<p>Body</p>"), ViewerTheme.light(), FontSize.of(18));

            TestAssertions.assertContains(
                    page, "--localmd-body-font-size:18px;", "page CSS variables must use selected paragraph font size");
            TestAssertions.assertContains(page, "p{font-size:var(--localmd-body-font-size)",
                    "paragraph CSS must follow the live font size variable");
        }

        @Test
        void appliesFontSizeToHeadings() {
            String page =
                    HtmlPageBuilder.buildPage(SafeHtml.fromTrustedRendererOutput("<h1>Title</h1><h2>Section</h2>"),
                            ViewerTheme.light(), FontSize.of(18));

            TestAssertions.assertContains(
                    page, "--localmd-h1-font-size:26px;", "h1 CSS variable must scale from selected font size");
            TestAssertions.assertContains(
                    page, "--localmd-h2-font-size:23px;", "h2 CSS variable must scale from selected font size");
            TestAssertions.assertContains(
                    page, "h1{font-size:var(--localmd-h1-font-size)", "h1 CSS must follow the live font size variable");
            TestAssertions.assertContains(
                    page, "h2{font-size:var(--localmd-h2-font-size)", "h2 CSS must follow the live font size variable");
        }
        ...
        void providesFontSizeUpdateScriptForPinchWithoutReloading() {
            ...
            TestAssertions.assertContains(script, "s.setProperty('--localmd-body-font-size','24px');",
                    "pinch font updates must update paragraph size without reloading the page");
            TestAssertions.assertContains(script, "s.setProperty('--localmd-h1-font-size','32px');",
                    "pinch font updates must update heading size without reloading the page");
            TestAssertions.assertContains(script, "s.setProperty('--localmd-diagram-scale','1.500');",
                    "pinch font updates must update Mermaid diagram scale without reloading the page");
        }
        ...
        void appliesFontSizeScaleToMermaidDiagrams() {
            String page = HtmlPageBuilder.buildPage(
                    ...
                    ViewerTheme.light(), FontSize.of(24));
            ...
            TestAssertions.assertContains(page,
                    ...
                    "Mermaid SVG internal text must not inherit the page font size before scaling");
        }
    }

    @Nested
    final class ReadableElements {
        @Test
        void stylesLinksWithThemeReadableColor() {
            ...
            TestAssertions.assertContains(lightPage, "a{color:#0b6f87;text-decoration:underline;}",
                    "light theme links must be visibly styled");
            TestAssertions.assertContains(
                    darkPage, "a{color:#8bd7ff;text-decoration:underline;}", "dark theme links must be visibly styled");
        }
        ...
        void stylesChecklistForReadableMobileLayout() {
            String page = HtmlPageBuilder.buildPage(
                    SafeHtml.fromTrustedRendererOutput(
                            "<ul class=\"checklist\"><li><input type=\"checkbox\" disabled> Done</li></ul>"),
                    ViewerTheme.light(), FontSize.of(18));

            TestAssertions.assertContains(page, "ul.checklist{list-style:none;padding-left:0;}",
                    "checklist CSS must remove duplicate bullets and excess left padding");
            TestAssertions.assertContains(page, "ul.checklist input{margin-right:8px;}",
                    "checklist checkbox must keep readable spacing from text");
        }
    }
    ...
    final class WelcomeHome {
        ...
        void stylesWelcomeHomeForFirstRun() {
            ...
            TestAssertions.assertContains(page, ".welcome-grid{display:grid;gap:10px;margin:0 0 18px;}",
                    "welcome home must lay out guidance as a stable grid");
            TestAssertions.assertContains(page,
                    ".welcome-card{background:#ffffff;border:1px solid #c9d8d5;border-radius:8px;padding:14px;}",
