...
public final class ViewerThemeStyleTest {
    @Nested
    final class ThemeIdentity {
        @Test
        void lightThemeStyleProvidesSharedBackgroundColor() {
            ViewerThemeStyle style = ViewerThemeStyle.from(ViewerTheme.light());

            TestAssertions.assertEquals(
                    "#f8fbfa", style.background, "Light theme native background must come from the shared theme style");
            TestAssertions.assertEquals(
                    "#f8fbfa", style.cssBackground, "Light theme CSS background must come from the shared theme style");
        }
        ...
        void gradientThemeStyleProvidesNativeColorAndCssGradient() {
            ...
            TestAssertions.assertEquals(
                    "#f3f7ff", style.background, "Gradient theme native background must keep a solid color fallback");
            TestAssertions.assertEquals("linear-gradient(135deg,#f3f7ff 0%,#d9f2ec 32%,#ffe2d1 68%,#f8f0ff 100%)",
                    style.cssBackground, "Gradient theme CSS background must come from the shared theme style");
        }
        ...
        void gradientAndAuroraUseClearlyDifferentBackgrounds() {
            ...
            TestAssertions.assertNotEquals(gradient.background, aurora.background,
                    "Gradient and Aurora must not share the same native background");
            TestAssertions.assertNotEquals(gradient.cssBackground, aurora.cssBackground,
                    "Gradient and Aurora must not share the same CSS gradient");
            TestAssertions.assertNotEquals(
                    gradient.text, aurora.text, "Gradient and Aurora must use different text contrast rules");
        }
        ...
        void darkAndAmoledUseClearlyDifferentSurfaces() {
            ...
            TestAssertions.assertNotEquals(
                    dark.background, amoled.background, "Dark and AMOLED must not share the same native background");
            TestAssertions.assertNotEquals(
                    dark.surface, amoled.surface, "Dark and AMOLED must not share the same button surface");
        ...
        void proLightFamilyThemesServeDifferentReadingPurposes() {
            ...
            TestAssertions.assertNotEquals(
                    mist.background, dusk.background, "Mist and Dusk must not collapse into the same light background");
        }
        ...
        void auroraThemeStyleUsesReadableDarkNativeColors() {
            ...
            TestAssertions.assertEquals(
                    "#10211d", style.surface, "Aurora button surface must be dark enough for light text");
            TestAssertions.assertEquals("#10211d", style.tableCellBackground,
                    "Aurora table cells must hide the page gradient behind table content");
            TestAssertions.assertEquals(
                    "#4f7569", style.border, "Aurora borders must be visible against dark table cells");
            TestAssertions.assertEquals(
                    "#f2fff7", style.text, "Aurora native text must be light enough for dark surfaces");
        }
        ...
        void amoledThemeStyleUsesReadablePureBlackNativeColors() {
            ...
            TestAssertions.assertEquals(
                    "#f7fff9", style.text, "AMOLED native text must be light enough for black surfaces");
        }
    }
    ...
    @Nested
    final class TableAndDiagramColors {
        @Test
        void darkThemeStyleProvidesSharedTableScrollHint() {
            ViewerThemeStyle style = ViewerThemeStyle.from(ViewerTheme.dark());

            TestAssertions.assertEquals("#8fb8ad", style.tableScrollHint,
                    "Dark theme table scroll hint must come from the shared theme style");
            TestAssertions.assertEquals("0.58", style.tableScrollHintOpacity,
                    "Dark theme table scroll opacity must come from the shared theme style");
        }
        ...
        void darkThemeStyleProvidesLightDiagramTextAndLines() {
            ...
            TestAssertions.assertEquals("#202a31", style.diagramBackground,
                    "Dark Mermaid diagrams must use a dark background matching document surfaces");
            TestAssertions.assertEquals("#f4faf8", style.diagramText, "Dark Mermaid diagrams must use light text");
            TestAssertions.assertEquals(
                    "#a9c8c0", style.diagramLine, "Dark Mermaid diagrams must use visible light lines");
        }
        ...
        void auroraThemeStyleProvidesLightDiagramTextAndLines() {
            ...
            TestAssertions.assertEquals(
                    "#10211d", style.diagramBackground, "Aurora Mermaid diagrams must use an opaque dark background");
            TestAssertions.assertEquals("#f2fff7", style.diagramText, "Aurora Mermaid diagrams must use light text");
            TestAssertions.assertEquals("#a6d8c0", style.diagramLine,
                    "Aurora Mermaid diagrams must keep lines readable over dark surfaces");
        }
    }
}
