...
public final class FontPinchControllerTest {
    ...
    void ongoingPinchAppliesTemporaryZoomWithoutReloadingTheDocument() {
        ...
        TestAssertions.assertEquals(120, output.zoomPercent,
                "ongoing pinch must immediately reflect the visible scale");
    }
    ...
    void invalidPinchScaleDoesNotIssueAVisualUpdate() {
        ...
        TestAssertions.assertEquals(0, output.zoomCalls,
                "invalid pinch input must not mutate the visible document");
    }
    ...
    void completedFontChangeRequestsOneDocumentRender() {
        ...
        TestAssertions.assertEquals(1, output.renderCalls,
                "completed font change must commit through one document render");
    }
    ...
    void completedPinchWithoutFontChangeResetsTemporaryZoom() {
        ...
        TestAssertions.assertEquals(100, output.zoomPercent,
                "pinch without a committed size change must return to normal zoom");
    }
    ...
    void loadedPageRestoresThePendingScrollPosition() {
        ...
        TestAssertions.assertEquals(175, output.restoredScrollY,
                "loaded page must restore the position requested when pinch finished");
    }
    ...
    void pendingScrollPositionIsConsumedAfterOnePageLoad() {
        ...
        TestAssertions.assertEquals(1, output.restoreCalls,
                "pending scroll restoration must not leak into later page loads");
    }
