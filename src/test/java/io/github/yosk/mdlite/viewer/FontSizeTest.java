    ...
    final class StepChanges {
        ...
        void doesNotIncreaseBeyondMaximum() {
            ...
            TestAssertions.assertEquals(
                    FontSize.MAX_SP, fontSize.sp(), "increasing maximum font size must stay at maximum");
        }
        ...
        void doesNotDecreaseBelowMinimum() {
            ...
            TestAssertions.assertEquals(
                    FontSize.MIN_SP, fontSize.sp(), "decreasing minimum font size must stay at minimum");
        }
    }

    @Nested
    final class PinchChanges {
        @Test
        void pinchingOutScalesFromTheInitialFontSize() {
            FontSize fontSize = FontSize.of(16).changedByPinchScale(1.20f);

            TestAssertions.assertEquals(
                    19, fontSize.sp(), "pinching out must keep the final size close to the visible pinch scale");
        }
        ...
        void pinchingInScalesFromTheInitialFontSize() {
            ...
            TestAssertions.assertEquals(
                    13, fontSize.sp(), "pinching in must keep the final size close to the visible pinch scale");
        }
        ...
        void pinchingOutCannotExceedMaximumFontSize() {
            ...
            TestAssertions.assertEquals(
                    FontSize.MAX_SP, fontSize.sp(), "pinching out must be clamped to the maximum font size");
        }
        ...
        void pinchingInCannotGoBelowMinimumFontSize() {
            ...
            TestAssertions.assertEquals(
                    FontSize.MIN_SP, fontSize.sp(), "pinching in must be clamped to the minimum font size");
        }
        ...
        void pinchingWithTinyMovementKeepsCurrentSize() {
            ...
            TestAssertions.assertEquals(
                    16, fontSize.sp(), "tiny pinch changes must not unexpectedly change the final font size");
        }
        ...
        void pinchingWithNaNScaleKeepsCurrentSize() {
            ...
            TestAssertions.assertEquals(
                    18, fontSize.sp(), "NaN pinch scale must be ignored to keep the current valid font size");
        }
        ...
        void pinchingWithZeroScaleKeepsCurrentSize() {
            ...
            TestAssertions.assertEquals(
                    18, fontSize.sp(), "zero pinch scale must be ignored to keep the current valid font size");
        }
        ...
        void pinchingWithNegativeScaleKeepsCurrentSize() {
            ...
            TestAssertions.assertEquals(
                    18, fontSize.sp(), "negative pinch scale must be ignored to keep the current valid font size");
        }
        ...
        void pinchingWithInfiniteScaleKeepsCurrentSize() {
            ...
            TestAssertions.assertEquals(
                    18, fontSize.sp(), "infinite pinch scale must be ignored to keep the current valid font size");
        }
    }

    @Nested
    final class PinchScaleValidity {
        @Test
        void positiveFinitePinchScaleCanBeAppliedToAnOngoingGesture() {
            TestAssertions.assertTrue(
                    FontSize.canApplyPinchScale(1.05f), "positive finite pinch scale must be accepted");
        }

        @Test
        void nanPinchScaleCannotBeAppliedToAnOngoingGesture() {
            TestAssertions.assertFalse(FontSize.canApplyPinchScale(Float.NaN),
                    "NaN pinch scale must not corrupt the accumulated pinch scale");
        }

        @Test
        void zeroPinchScaleCannotBeAppliedToAnOngoingGesture() {
            TestAssertions.assertFalse(
                    FontSize.canApplyPinchScale(0.0f), "zero pinch scale must not corrupt the accumulated pinch scale");
        }

        @Test
        void negativePinchScaleCannotBeAppliedToAnOngoingGesture() {
            TestAssertions.assertFalse(FontSize.canApplyPinchScale(-1.0f),
                    "negative pinch scale must not corrupt the accumulated pinch scale");
        }

        @Test
        void infinitePinchScaleCannotBeAppliedToAnOngoingGesture() {
            TestAssertions.assertFalse(FontSize.canApplyPinchScale(Float.POSITIVE_INFINITY),
                    "infinite pinch scale must not corrupt the accumulated pinch scale");
        }
    }
}
