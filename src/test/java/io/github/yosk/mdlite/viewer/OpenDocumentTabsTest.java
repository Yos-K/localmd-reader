...
public final class OpenDocumentTabsTest {
    ...
    final class Opening {
        ...
        void openExistingUriReplacesThatTabAndActivatesItWithoutDuplicate() {
            ...
            TestAssertions.assertEquals(
                    3, tabs.tabs().size(), "opening an existing URI must not create a duplicate tab");
            TestAssertions.assertEquals(
                    1, tabs.activeIndex(), "reopened existing tab must become active at its original position");
            TestAssertions.assertEquals(
                    "First Updated", tabs.activeTab().title(), "existing tab must use latest title");
            TestAssertions.assertEquals(
                    "new", tabs.activeTab().document().value(), "existing tab must use latest rendered document");
        }
    }
    ...
    final class Activation {
        ...
        void activateSwitchesTheActiveTabByIndex() {
            ...
            TestAssertions.assertEquals(
                    "Welcome", tabs.activeTab().title(), "activating a tab index must switch the active document");
        }
        ...
        void activatePreviousSwitchesToThePreviousTab() {
            ...
            TestAssertions.assertEquals(
                    "First", tabs.activeTab().title(), "activating previous must switch to the tab on the left");
        }
        ...
        void activatePreviousWrapsFromFirstToLastTab() {
            ...
            TestAssertions.assertEquals("Second", tabs.activeTab().title(),
                    "activating previous from the first tab must wrap to the last tab");
        }
        ...
        void activateNextSwitchesToTheNextTab() {
            ...
            TestAssertions.assertEquals(
                    "Second", tabs.activeTab().title(), "activating next must switch to the tab on the right");
        }
        ...
        void activateNextWrapsFromLastToFirstTab() {
            ...
            TestAssertions.assertEquals("Welcome", tabs.activeTab().title(),
                    "activating next from the last tab must wrap to the first tab");
        }
    }

    @Nested
    final class RenderedDocumentReplacement {
        @Test
        void replaceRenderedDocumentKeepsTheCurrentlyActiveTab() {
            OpenDocumentTabs tabs = threeTabs().activate(2).replaceRenderedDocument(
                    "content://first", SafeHtml.fromTrustedRendererOutput("updated"));

            TestAssertions.assertEquals(
                    "Second", tabs.activeTab().title(), "re-rendering an inactive tab must preserve the active tab");
            TestAssertions.assertEquals("updated", tabs.tabs().get(1).document().value(),
                    "re-rendering must replace the targeted tab document");
        }
        ...
        void replaceRenderedDocumentForUnknownUriKeepsTheTabSessionUnchanged() {
            ...
            OpenDocumentTabs unchanged =
                    tabs.replaceRenderedDocument("content://missing", SafeHtml.fromTrustedRendererOutput("unused"));
    ...
    final class Closing {
        ...
        void closeInactiveTabKeepsCurrentActiveDocument() {
            OpenDocumentTabs tabs =
                    threeTabs().activate(2).closeOrFallback(1, tab("Welcome", "app://welcome", "welcome")).tabs();

            TestAssertions.assertEquals(2, tabs.tabs().size(), "closing an inactive tab must remove one tab");
            TestAssertions.assertEquals("Second", tabs.activeTab().title(),
                    "closing an inactive tab before the active tab must keep the same active document");
        }

        @Test
        void closeActiveMiddleTabActivatesTheNextDocument() {
            OpenDocumentTabs tabs =
                    threeTabs().activate(1).closeOrFallback(1, tab("Welcome", "app://welcome", "welcome")).tabs();

            TestAssertions.assertEquals(2, tabs.tabs().size(), "closing the active middle tab must remove one tab");
            TestAssertions.assertEquals("Second", tabs.activeTab().title(),
                    "closing the active middle tab must activate the next document");
        }

        @Test
        void closeActiveLastTabActivatesThePreviousDocument() {
            OpenDocumentTabs tabs =
                    threeTabs().activate(2).closeOrFallback(2, tab("Welcome", "app://welcome", "welcome")).tabs();

            TestAssertions.assertEquals(2, tabs.tabs().size(), "closing the active last tab must remove one tab");
            TestAssertions.assertEquals("First", tabs.activeTab().title(),
                    "closing the active last tab must activate the previous document");
        }
        ...
        void closeOrFallbackReplacesTheOnlyOpenDocumentWithTheFallbackTab() {
            ...
            TestAssertions.assertEquals(
                    1, tabs.tabs().size(), "closing the only document with fallback must keep one tab open");
            TestAssertions.assertEquals("Welcome", tabs.activeTab().title(),
                    "closing the only document with fallback must show the fallback tab");
            TestAssertions.assertEquals(
                    "app://welcome", tabs.activeTab().uri(), "fallback tab must become the only active tab");
        }
        ...
        void closeResultRemovesClosedDocumentFromItsRenderingSession() {
            ...
            DocumentRenderingSession rendering = tabs.closeOrFallback(0, tab("Welcome", "app://welcome", "welcome"))
        ...
        void invalidCloseResultKeepsTheTabSessionUnchanged() {
            ...
            OpenDocumentTabs unchanged = tabs.closeOrFallback(99, tab("Welcome", "app://welcome", "welcome")).tabs();

            TestAssertions.assertSame(
                    tabs, unchanged, "an invalid close position must preserve the existing tab session");
        }

        @Test
        void invalidCloseResultKeepsTheRenderingSessionUnchanged() {
            OpenDocumentTabs tabs = threeTabs();
            DocumentRenderingSession rendering =
                    DocumentRenderingSession.empty()
                            .open(DocumentUri.from("content://first"), "# First", freeProfile())
                            .session();

            DocumentRenderingSession unchanged = tabs.closeOrFallback(99, tab("Welcome", "app://welcome", "welcome"))
                                                         .renderingSessionAfter(rendering);

            TestAssertions.assertSame(
                    rendering, unchanged, "an invalid close position must preserve the existing rendering session");
        }
    }
