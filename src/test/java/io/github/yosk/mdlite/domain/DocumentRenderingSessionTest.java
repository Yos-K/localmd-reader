...
public final class DocumentRenderingSessionTest {
    ...
    final class Opening {
        ...
        void openingDocumentProducesOneAlwaysValidRenderInput() {
            DocumentOpeningPlan plan = DocumentRenderingSession.empty().open(DOCUMENT_URI, null, freeProfile());
        ...
        void openingMermaidDocumentSchedulesItsDiagram() {
            ...
            TestAssertions.assertEquals(
                    1, plan.jobs().length, "opening Mermaid Markdown must schedule its diagram once");
        }
    ...
    final class DiagramCompletion {
        ...
        void completedDiagramProducesAnUpdatedRenderInput() {
            ...
            TestAssertions.assertSame(svg, capture.input.renderedMermaidDiagrams().get(Integer.valueOf(0)),
                    "accepted Mermaid completion must produce a render input "
                            + "containing its SVG");
        }
        ...
        void staleCompletionProducesNoRenderInput() {
            ...
            DocumentRenderingCompletion stale = newTheme.session().complete(
                    oldTheme.jobs()[0], SafeHtml.fromTrustedRendererOutput("<svg>stale</svg>"));
    ...
    final class ThemeReset {
        ...
        void themeResetSchedulesEveryKnownDiagramAgain() {
            ...
            TestAssertions.assertEquals(
                    1, reset.jobs().length, "theme reset must schedule every known diagram for the new theme");
        }

        @Test
        void themeResetProducesRenderInputForEveryKnownDocument() {
            DocumentOpeningPlan first = openMermaidDocument();
            DocumentOpeningPlan second =
                    first.session().open(DocumentUri.from("content://notes"), "# Notes", freeProfile());

            DocumentRenderingBatchPlan reset = second.session().resetForTheme(freeProfile());

            TestAssertions.assertEquals(
                    2, reset.renderInputs().length, "theme reset must refresh every known document");
        }
    }
    ...
    final class Closing {
        ...
        void closingDocumentPreventsItsDiagramFromBeingScheduledAgain() {
            ...
            TestAssertions.assertEquals(
                    0, reset.jobs().length, "closed document diagrams must not be scheduled after a theme reset");
        }
        ...
        void completingClosedDocumentDiagramProducesNoRenderInput() {
            ...
            DocumentRenderingCompletion completed =
                    closed.complete(opened.jobs()[0], SafeHtml.fromTrustedRendererOutput("<svg>late</svg>"));
            completed.dispatch(count);

            TestAssertions.assertEquals(0, count.value, "late diagram completion must not revive a closed document");
        }
        ...
        void closingUnknownDocumentKeepsTheRenderingSessionUnchanged() {
            ...
            TestAssertions.assertSame(opened.session(), unchanged,
                    "closing an unknown document must preserve "
                            + "the existing rendering session");
        }
    }
    ...
    private static final class CapturedRenderInput implements DocumentRenderingCompletion.Handler {
        ...
        @Override
        public void rendered(DocumentRenderInput input) {
            this.input = input;
        }
        @Override
