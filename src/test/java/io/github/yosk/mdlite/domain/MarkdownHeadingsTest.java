...
public final class MarkdownHeadingsTest {
    ...
    final class BasicExtraction {
        ...
        void repeatedHeadingTitlesReceiveStableUniqueAnchors() {
            ...
            TestAssertions.assertEquals(
                    "intro", headings.at(0).anchorId(), "first repeated heading keeps the base anchor");
            TestAssertions.assertEquals(
                    "intro-2", headings.at(1).anchorId(), "second repeated heading receives numeric suffix");
            TestAssertions.assertEquals(
                    "intro-3", headings.at(2).anchorId(), "third repeated heading receives numeric suffix");
        }
    }

    @Nested
    final class BasicFencedCodeHandling {
        @Test
        void codeBlockHashesAreIgnored() {
            MarkdownHeadings headings = MarkdownHeadings.fromMarkdown("# Real\n```md\n# Not heading\n```\n## Next");

            TestAssertions.assertEquals(
                    2, headings.count(), "Markdown headings must ignore hash lines inside fenced code blocks");
    ...
    final class HeadingValidity {
        ...
        void hashWithoutSpaceIsNotAHeading() {
            ...
            TestAssertions.assertEquals(
                    1, headings.count(), "hash without trailing space must not be parsed as heading");
    ...
    @Nested
    final class LineEndingCompatibility {
        @Test
    ...
    final class AnchorRegressions {
        ...
        void nonAsciiHeadingTitlesCollapseToTheHeadingAnchorAndAreNumbered() {
            ...
            TestAssertions.assertEquals(
                    "heading", headings.at(0).anchorId(), "first non-ASCII heading falls back to the heading anchor");
            TestAssertions.assertEquals(
                    "heading-2", headings.at(1).anchorId(), "second non-ASCII heading is numbered apart");
        }
        ...
        void titlesDifferingOnlyByCaseReceiveUniqueAnchors() {
            ...
            TestAssertions.assertEquals(
                    "foo", headings.at(0).anchorId(), "first heading keeps the case-folded base anchor");
            TestAssertions.assertEquals("foo-2", headings.at(1).anchorId(), "case-only variant is numbered apart");
        }
    }

    @Nested
    final class FenceBoundaryRegressions {
        ...
        void indentedCodeFenceSuppressesInnerHeadings() {
            MarkdownHeadings headings =
                    MarkdownHeadings.fromMarkdown("# Real\n   ```\n# Not heading\n   ```\n## After");

            TestAssertions.assertEquals(
                    2, headings.count(), "up-to-3-space indented fences must suppress inner hash lines");
        ...
        void closingFenceWithTrailingSpaceStillClosesTheBlock() {
            ...
            TestAssertions.assertEquals(
                    2, headings.count(), "a trailing-space closing fence must still close the code block");
        ...
        void fourSpaceIndentIsNotACodeFence() {
            ...
            TestAssertions.assertEquals(
                    2, headings.count(), "4-space indent is not a fence, so the following heading still counts");
            TestAssertions.assertEquals(
                    "After", headings.at(1).title(), "heading after a 4-space-indented backtick line");
        }
    }
}
