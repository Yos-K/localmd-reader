...
public final class MermaidRenderSessionsTest {
    ...
    final class Scheduling {
        ...
        void registeredDocumentSchedulesEveryDiagramForBackgroundRendering() {
            ...
            TestAssertions.assertEquals(
                    2, schedule.jobs().length, "every registered Mermaid block must be scheduled once");
        }
    ...
    @Nested
    final class CallbackMatching {
        @Test
        void scheduledJobMatchesOnlyItsOwnCallbackCoordinates() {
            MermaidRenderJob job = registeredSession().schedule(DOCUMENT_URI).jobs()[0];

            TestAssertions.assertFalse(job.matches(DOCUMENT_URI.value(), 1),
                    "a callback for another diagram must not complete the active job");
        }
        ...
        void scheduledJobAcceptsItsOwnCallbackCoordinates() {
            ...
            TestAssertions.assertTrue(
                    job.matches(DOCUMENT_URI.value(), 0), "the matching callback must complete the active job");
        }
    }

    @Nested
    final class CompletionAndGenerationIsolation {
        @Test
        ...
        void resetMakesEveryDiagramEligibleForTheNewTheme() {
            ...
            MermaidRenderSessions completed = initial.session().complete(
                    initial.jobs()[0], SafeHtml.fromTrustedRendererOutput("<svg>old theme</svg>"));

            MermaidRenderSchedule reset = completed.resetRendered().schedule(DOCUMENT_URI);

            TestAssertions.assertEquals(
                    2, reset.jobs().length, "theme reset must schedule every diagram with the new theme");
        }
        ...
        void completionFromThePreviousThemeCannotReplaceCurrentResults() {
            ...
            MermaidRenderSessions unchanged =
                    newTheme.complete(oldTheme.jobs()[0], SafeHtml.fromTrustedRendererOutput("<svg>stale</svg>"));

            TestAssertions.assertSame(
                    newTheme, unchanged, "stale Mermaid completion must not mutate the current render generation");
        }
        ...
        void registeringUpdatedMarkdownInvalidatesPriorDiagramResults() {
            MermaidRenderSchedule initial = registeredSession().schedule(DOCUMENT_URI);
            MermaidRenderSessions completed = initial.session().complete(
                    initial.jobs()[0], SafeHtml.fromTrustedRendererOutput("<svg>old source</svg>"));

            MermaidRenderSchedule updated =
                    completed
                            .register(
                                    DOCUMENT_URI, MermaidDiagramBlocks.fromMarkdown("```mermaid\ngraph BT\nC-->D\n```"))
                            .schedule(DOCUMENT_URI);

            TestAssertions.assertEquals(1, updated.jobs().length,
                    "updated Markdown must replace prior results and schedule its current blocks");
        }
    }

    private static MermaidRenderSessions registeredSession() {
        return MermaidRenderSessions.empty().register(DOCUMENT_URI,
